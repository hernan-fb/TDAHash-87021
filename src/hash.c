...
lista_iterador_t *hash_buscar(const hash_t *hash, const char *clave) {
  ...
  puntero_funcion_hash_t array_funciones_hash[5] = {hash_01, hash_02, hash_03,
                                                    hash_04, NULL};
  size_t posicion =
      array_funciones_hash[FUNCION_HASH_UTILIZADA](clave, hash->tamanio);
...
hash_t *hash_insertar(hash_t *hash, const char *clave, void *elemento,
  ...
  if (clave_valor_en_iter) {
    if (anterior) {
      *anterior =
          ((clave_valor_t *)lista_iterador_elemento_actual(clave_valor_en_iter))
              ->valor;
    }
    ((clave_valor_t *)lista_iterador_elemento_actual(clave_valor_en_iter))
        ->valor = elemento;
...
hash_t *hash_guardar_clave_valor(hash_t *hash, clave_valor_t *clave_valor) {
  ...
  if (lista_insertar_en_posicion(hash->tabla[resultado_hash], clave_valor,
...
size_t hash_01(const void *clave, size_t largo_tabla_hash) {
  ...
  for (size_t i = 0; i < n; i++) {
    hash = hash ^ (((unsigned char *)
                        clave)[i]); // xor next byte into the bottom of the hash
...
size_t hash_04(const void *clave, size_t largo_tabla_hash) {
  ...
  const uint_fast64_t *end =
      data + (strlen(clave) / 8); // originariamente decía data + (largo/8);
  // size_t i = 0;
...
size_t busquedabinaria(size_t *array, size_t inicio, size_t fin,
  ...
  if (array[medio] == elemento_buscado)
    return medio;
  if (array[medio] < elemento_buscado)
    return busquedabinaria(array, (size_t)medio + 1, fin, elemento_buscado,
                           aumenta);
  else
    return busquedabinaria(array, inicio, (size_t)medio - 1, elemento_buscado,
                           aumenta);
}
size_t calcula_nuevo_tamanio(hash_t *hash, bool aumenta_el_tamanio) {
  ...
  size_t posicion_aux =
      busquedabinaria(lista_numeros_primos, 0, LARGO_LISTADO_NUMERO_PRIMOS - 1,
                      hash->tamanio, aumenta_el_tamanio);
  if (aumenta_el_tamanio) {
    ...
    if (posicion_aux == 0) {
      ...
      nuevo_tamanio = hash->tamanio / 10;
      if (nuevo_tamanio <=
          lista_numeros_primos[LARGO_LISTADO_NUMERO_PRIMOS - 1]) {
...
bool chequear_factor_de_carga_y_redimensionar(hash_t *hash,
  ...
  if (!aumenta_el_tamanio) {
    ...
    if (nuevo_factor_de_carga <
        FACTOR_CARGA_MAXIMO * COEFICIENTE_REDIMENSION_POR_FACTOR_DE_CARGA) {
