...
bool chequear_factor_de_carga_y_redimensionar(hash_t *hash, bool aumenta_el_tamanio);
...
lista_iterador_t* hash_buscar(const hash_t *hash, const char *clave) {
	...
	size_t posicion = array_funciones_hash[FUNCION_HASH_UTILIZADA](clave, hash->tamanio);
		      ...
		      void **anterior)
{
	...
	if (clave_valor_en_iter) {
		if (anterior) {
			*anterior = ((clave_valor_t*)lista_iterador_elemento_actual(clave_valor_en_iter))->valor;
		}
		((clave_valor_t*)lista_iterador_elemento_actual(clave_valor_en_iter))->valor = elemento;
...
hash_t * hash_guardar_clave_valor(hash_t *hash, clave_valor_t *clave_valor) {
	puntero_funcion_hash_t array_funciones_hash[5] = {hash_01, hash_02, hash_03, hash_04, NULL};
	size_t resultado_hash = array_funciones_hash[FUNCION_HASH_UTILIZADA](clave_valor->clave, hash->tamanio);
	if(!hash->tabla[resultado_hash]) {
		hash->tabla[resultado_hash] = lista_crear();
	}
	hash->cantidad++;
	//ACAprintf("incremento cantidad ahora es: %zu\n", hash_cantidad(hash));
	if(lista_insertar_en_posicion(hash->tabla[resultado_hash], clave_valor, lista_tamanio(hash->tabla[resultado_hash]))) {
		return hash;
	}
	return NULL;
}
...
void *hash_quitar(hash_t *hash, const char *clave)
{
	...
	if (iter) {
		valor = ((clave_valor_t*)lista_iterador_elemento_actual(iter))->valor;
		destruir_estructura_clave_pero_no_el_valor(lista_iterador_borrar(iter));
		hash->cantidad--;
...
void *hash_obtener(hash_t *hash, const char *clave)
{
	...
	if (dato_en_iter) {
		clave_valor_t* actual = lista_iterador_elemento_actual(dato_en_iter);
...
void hash_destruir_todo(hash_t *hash, void (*destructor)(void *))
{
    ...
    for (size_t i = 0; i < hash->tamanio; i++) {
		...
		while (lista_iterador_tiene_siguiente(iter)){
			clave_valor_t* clave_valor = lista_iterador_borrar(iter);
			if (destructor) destructor(clave_valor->valor);
			   ...
			   void *aux)
{
    ...
    for (size_t i = 0; i < hash->tamanio; i++) {
		...
		while (lista_iterador_tiene_siguiente(iter)){
			...
			n++;
			void *aux = NULL;
			if (clave_valor->clave == NULL) printf("la clave es NUll\n");
			if (clave_valor->valor == NULL) printf("el valor es NUll\n");			
			printf("estoy en la lista %zu, en la posicion %zu, la clave es %p y el valor es %p\n", i, borrar_este_contador+1, (char*)clave_valor->clave, (void*)clave_valor->valor);
			bool resultado_de_funcion = f((char*)clave_valor->clave, clave_valor->valor, aux);
			printf("asdf2\n");
			if (!resultado_de_funcion) break;
			printf("asdf3\n");
			lista_iterador_avanzar(iter);
			borrar_este_contador++;
			//printf("estoy en la lista %zu, en la posicion %zu, la clave es %s y el valor es %s\n", i, borrar_este_contador, (char*)clave_valor->clave, (char*)clave_valor->valor);
		}
...
size_t hash_04(const void * clave, size_t largo_tabla_hash) {
	...
	const uint_fast64_t * end = data + (strlen(clave)/8); //originariamente decía data + (largo/8);
	//size_t i = 0;
...
size_t busquedabinaria(size_t *array, size_t inicio, size_t fin, size_t elemento_buscado, bool aumenta ) {
	if (inicio > fin) return aumenta?fin:inicio;
	size_t medio = (inicio + fin ) / 2;

	if (array[medio] == elemento_buscado ) 	return medio;
	if (array[medio] < elemento_buscado ) 	return busquedabinaria(array,(size_t)medio+1,fin,elemento_buscado, aumenta);
	else 									return busquedabinaria(array,inicio,(size_t)medio-1,elemento_buscado, aumenta);
}
size_t calcula_nuevo_tamanio(hash_t *hash, bool aumenta_el_tamanio) {
	size_t nuevo_tamanio;
	size_t lista_numeros_primos[LARGO_LISTADO_NUMERO_PRIMOS] = {3, 37, 197, 1973, 19709, 197003, 999983};

	size_t posicion_aux = busquedabinaria(lista_numeros_primos,0,LARGO_LISTADO_NUMERO_PRIMOS-1,hash->tamanio, aumenta_el_tamanio);
	if (aumenta_el_tamanio) {
		...
		if (posicion_aux == 0) {
			...
			nuevo_tamanio = hash->tamanio / 10;
			if (nuevo_tamanio <= lista_numeros_primos[LARGO_LISTADO_NUMERO_PRIMOS-1]) {
				nuevo_tamanio = lista_numeros_primos[LARGO_LISTADO_NUMERO_PRIMOS-1];
			}
		}
	}
...
bool chequear_factor_de_carga_y_redimensionar(hash_t *hash, bool aumenta_el_tamanio) {
	...
	if ( !aumenta_el_tamanio ) {
		...
		float nuevo_factor_de_carga = (float) aux / (float) (nuevo_tamanio * CAPACIDAD_BUCKETS);
		if (nuevo_factor_de_carga < FACTOR_CARGA_MAXIMO * COEFICIENTE_REDIMENSION_POR_FACTOR_DE_CARGA) {
...
//--- copio la tabla actual en un hash aux porque hash_guardar conserva el mismo hash---//
...
//---					---------						---//
    ...
    for (size_t i = 0; i < hash_aux->tamanio; i++) {
		...
		while (lista_iterador_tiene_siguiente(iter_aux)){
			clave_valor_t* clave_valor = lista_iterador_borrar(iter_aux);
			hash_aux->cantidad--;
			guardados_ok &= (hash_guardar_clave_valor(hash, clave_valor) != NULL);
			if (!guardados_ok) {
				return false;
			}
		}
